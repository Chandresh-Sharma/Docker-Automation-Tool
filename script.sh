...
do
    ...
    case $choice in
        ...
        8)
            ...
            while docker ps -a --format "{{.Names}}" | grep -w "${containerName}" > /dev/null
            do
                echo "Name exists"
        ...
        10)
            ...
            then 
                echo "No container is in a running state"
            else
...
case $proChoice in
    1)    
        read -p "Enter the task you want to perform related to Docker Management:" prompt
        echo "Handling Docker Management task: $prompt"
        ;;
